...
def _load_parquet_raw(path: str) -> pd.DataFrame:
    ...
    df["COMMODITY_TAG"] = df["HS4"].map(HS4_LABELS).fillna("Other Cotton")
    return df
...
with tab1:
    ...
    _fk = {"Cotton Exports": "exp"}[flow_choice]
    ...
    st.caption(
        ...
        f"HS Chapter 52 cotton headings: 5201-5212"
    )
...
with tab4:
    ...
    st.markdown(
        "<div style='text-align:center;padding:60px 0'>"
        "<span style='font-size:1.2rem;font-weight:500;color:#3a3a3c'>Mirror</span><br>"
        "<span style='font-size:0.85rem;color:#8e8e93'>Disabled for now - current cotton dataset is TDM exports only</span>"
        "</div>",
        unsafe_allow_html=True,
    )
